...
const date=new Date(Date.now()+86400000).toISOString().slice(0,10);const availability=await fetch(`${origin}/api/booking/availability?date=${date}`);const data=await availability.json();if(!availability.ok||!data.connected||!Array.isArray(data.slots))throw Error('Booking availability failed');if(environment==='staging'&&data.testMode!==true)throw Error('Staging is not isolated');if(environment==='production'&&data.testMode)throw Error('Production exposes test mode');
